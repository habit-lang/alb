...
module Printer.WadlerLeijen
        ...
        , cat, fillCat, hcat, vcat, vcatOr
        , punctuate

        , align, hang, indent
        , fill, fillBreak

        , list, tupled, semiBraces, commaBraces, encloseSep
        ...
        , softline, softbreak
        , empty, char, text, line, linebreak, lineOr, nest, group
...
hcat            = fold (<>)
vcat            = fold (<$$>)
vcatOr s        = fold (\x y -> x <> lineOr s <> y)
...
data Doc        = Empty
                ...
                | Line String           -- When undone by group, insert given string
                | Cat Doc Doc
                | Nest !Int Doc
...
flatten (Line s)        = if null s then Empty else Text (length s) s
